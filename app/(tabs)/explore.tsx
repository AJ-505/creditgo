...
export default function ExploreScreen() {
  ...
  const [expandedCategory, setExpandedCategory] =
    useState<AssetCategory | null>((params.category as AssetCategory) || null);
  ...
  const partnersByCategory = ASSET_CATEGORIES.reduce(
    (acc, category) => {
  ...
  const totalAffordable = PARTNERS.filter(
    ...
    <SafeAreaView className="flex-1 bg-slate-50">
      ...
      <View className="px-5 pt-2 pb-4">
        ...
        <Text className="text-sm text-slate-500 mb-4">
          {totalAffordable} partners match your budget of{" "}
      ...
      >
        {ASSET_CATEGORIES.map((category) => {
          ...
          const affordableCount = partners.filter(
            (p) => (p.minPayment || 0) <= safeAmount,
          ).length;
      ...
      <View className="flex-1">
        ...
        <View className="flex-row items-center justify-between">
          <Text className="text-slate-500 text-xs">
            ...
            {formatNaira(partner.maxPayment || 0)}/mo
          </Text>
          {isAffordable ? (
            <View className="flex-row items-center">
