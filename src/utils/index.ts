...
export const validateEmailFormat = (email: string): EmailValidationResult => {
  ...
  if (parts.length !== 2) {
    return {
      isValid: false,
      error: 'Email address should contain only one "@" symbol',
    };
  }
  ...
  if (!domain || domain.length === 0) {
    return {
      isValid: false,
      error: 'Please enter a domain after the "@" symbol',
    };
  }
...
export const validateCorporateEmail = (
  email: string,
): { isValid: boolean; company: string | null } => {
 ...
 */
export const validateFreelanceLink = (
  url: string,
): { isValid: boolean; platform: string | null } => {
  try {
    ...
    if (matchedPlatform) {
      const platform =
        matchedPlatform.split(".")[0].charAt(0).toUpperCase() +
 ...
 */
export const parseSMSTransactions = (
  messages: Array<{ body: string; date: Date }>,
): SMSTransaction[] => {
  ...
  for (const msg of messages) {
    ...
    const amountMatch = msg.body.match(
      /NGN\s?([\d,]+\.?\d*)|N\s?([\d,]+\.?\d*)|([\d,]+\.?\d*)\s?naira/i,
    );
 ...
 */
export const analyzeSMSTransactions = (
  ...
  const averageMonthlyIncome =
    monthsObserved > 0 ? totalCredits / monthsObserved : totalCredits;
  ...
  const detectedSources = [
    ...new Set(credits.map((t) => t.source).filter(Boolean) as string[]),
  ];
...
export const calculateSafeAmount = (
  ...
  const baseIncome = Math.min(
    monthlyIncome,
    Math.max(verifiedIncome, monthlyIncome * 0.8),
  );
  ...
  const estimatedExpenses = monthlyExpenses || baseIncome * 0.6;
  ...
  const maxAmount = Math.max(
    safeAmount,
    Math.min(maxFromDisposable, baseIncome * 0.25),
  );
